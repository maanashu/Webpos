...
const Verify = () => {
  ...
  const selectedposusernfo = authData?.selectedUserData
    ? authData?.selectedUserData
    : "";
  ...
  useEffect(() => {
    if (posSecurityPin?.length == OTP_INPUT_COUNT) {
      enterPinSubmit();
    }
  }, [posSecurityPin]);
        ...
        cb(res) {
          if (res) {
            ...
            if (res?.data?.payload?.user_profiles?.is_two_fa_enabled === true) {
              ...
              res?.data?.payload?.user_profiles?.is_two_fa_enabled === false
            ) {
            ...
            <div className="verifyBox">
              <div className="loginCard">
                <figure className="loginIds">
                  <Image
                    src={
                      selectedposusernfo?.user?.user_profiles?.profile_photo
                        ? selectedposusernfo?.user?.user_profiles?.profile_photo
                        : Images.userDummy
                    }
                    alt="LoginIdImage"
                    width="100"
                    height="100"
                    className="img-fluid loginIdImg"
                  />
                </figure>
                <div className="loginMainHead">
                  <h2 className="loginMain">
                    {selectedposusernfo?.user?.user_profiles?.firstname}{" "}
                    {selectedposusernfo?.user?.user_profiles?.lastname}
                  </h2>

                  {selectedposusernfo?.user?.user_roles.length > 0 ? (
                    selectedposusernfo?.user?.user_roles?.map((data, index) => {
                      return <h4 className="loginSub">{data?.role?.name}</h4>;
                    })
                  ) : (
                  ...
                  {selectedposusernfo?.user?.api_tokens?.length > 0 ? (
                    <>
                      <h4 className="loginPara mt-3">
                        {moment(
                          selectedposusernfo?.user?.api_tokens[0]?.updated_at
                        ).fromNow()}
                      </h4>
                      <h4 className="loginPara ">
                        {moment(
                          selectedposusernfo?.user?.api_tokens[0]?.updated_at
                        ).format("LT")}
                      </h4>
